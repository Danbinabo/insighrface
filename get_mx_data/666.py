...
for j in range(0, len(same_list), 2):
    print(same_list[j])
    print(same_list[j+1])
    name_img = same_list[j].split('\\')[-1].split('_00')[0]
...
while True:
    ...
    for p in range(0, len(list2), 2):
        if list2[p].split('\\')[-1].split('_00')[0] != list2[p - 1].split('\\')[-1].split('_00')[0]:
            name_img_1 = list2[p].split('\\')[-1].split('_00')[0]
            num_1_1 = int(list2[p].split('\\')[-1].split('_')[-1].split('.png')[0])
            name_img_2 = list2[p - 1].split('\\')[-1].split('_00')[0]
